...
void FilTxManager::OnGetNextNonce(std::unique_ptr<FilTxMeta> meta,
  ...
  GetFilTxStateManager()->AddOrUpdateTx(*meta);
  DLOG(INFO) << "SendFilecoinTransaction:" << meta->id() << " :" << signed_tx;
...
void FilTxManager::OnSendFilecoinTransaction(
    ...
    mojom::FilecoinProviderError error,
    const std::string& error_message) {
  DLOG(INFO) << "OnSendFilecoinTransaction:" << error_message << " :" << tx_cid;
...
void FilTxManager::GetAllTransactionInfo(
  ...
  DLOG(INFO) << "GetAllTransactionInfo:" << from_address.EncodeAsString();
