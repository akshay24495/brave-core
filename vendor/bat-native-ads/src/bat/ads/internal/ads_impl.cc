...
void AdsImpl::OnSearchResultAdEvent(
    const std::string& uuid,
    mojom::SearchResultAdPtr search_result_ad,
    const mojom::SearchResultAdEventType event_type) {
  // TODO(tmancey): Persist value in database::table::AdValue database, and
  // refactor CashDeposit to get the redemption value from this database table.
  // i.e. search_result_ad_->Save(...)

  // TODO(tmancey): Persist conversion in database::table::Conversions. i.e.
  // search_result_ad_->Save(...)

  // TODO(tmancey): Build SearchResultAdInfo from mojom::SearchResultAdPtr as a
  // workaround until all ad types are transitioned to mojom
  SearchResultAdInfo ad;

  search_result_ad_->FireEvent(uuid, ad, event_type);
}
