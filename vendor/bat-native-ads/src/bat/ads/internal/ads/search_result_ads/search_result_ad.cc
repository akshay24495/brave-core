...
void SearchResultAd::FireEvent(
  ...
  if (uuid.empty() || ad.creative_instance_id.empty()) {
    BLOG(1, "Failed to fire search result ad event due to invalid uuid "
                << uuid << " or creative instance id "
                << ad.creative_instance_id);
    NotifySearchResultAdEventFailed(uuid, ad.creative_instance_id, event_type);
    return;
  }
  ...
  database_table.GetForType(
      ...
      [=](const bool success, const AdEventList& ad_events) {
        if (!success) {
          ...
          NotifySearchResultAdEventFailed(uuid, ad.creative_instance_id,
                                          event_type);
          return;
        }
        ...
        if (event_type == mojom::SearchResultAdEventType::kViewed) {
          ...
          FireEvent(uuid, ad, mojom::SearchResultAdEventType::kServed);
        }
